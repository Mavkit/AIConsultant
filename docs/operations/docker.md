...
The Docker baseline makes the EL Råger API reproducible across development, CI, and later deployment environments. The runtime image is multi-stage, runs as the unprivileged `node` user, contains production dependencies only, and exposes explicit health behavior.

## Prerequisites

- Docker Engine or Docker Desktop with Compose v2.
- Network access to the configured container registry and npm registry for the first build.
- Port 3001 available, or set `EL_RAGER_API_PORT` to another host port.
...
```bash
EL_RAGER_API_PORT=8080 docker compose up --build
```

PowerShell:

```powershell
$env:EL_RAGER_API_PORT = "8080"
docker compose up --build
```
...
Interactive API documentation is available at <http://localhost:3001/documentation>.
...
| Health | Image and Compose probe `/health` from inside the container |
...
CI runs type checks, tests, and the application build before building the API image. A failed compile, test, dependency install, or image build blocks the workflow.
...
Confirm that the API listens on `0.0.0.0:3001` and that `/health` returns HTTP 200 inside the container.
